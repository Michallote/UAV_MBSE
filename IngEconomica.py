...
# isr_bins_file = os.path.normpath(r'C:\Users\Michel Gordillo\Downloads\ISR_bins.txt')
...
# df_ISR = pd.DataFrame([lim_inf,lim_sup,cuota,percentage], index = ['LI','LS','Cuota','tasa_excedente']).transpose()
...
def det_niveles(base_salary):
    ...
    for i, (minimum, maximum) in enumerate(zip(lim_inf,lim_sup)):
        ...
        if base_salary >= minimum and base_salary <= maximum:
            nivel = i
            print('\nSueldo Base: {}\nNivel: {}°: \nLímite inferior: {}\nLímite superior: {}\nCuota: {}\nPorcentaje: {}\n'.format(base_salary,nivel+1,minimum,maximum,cuota[nivel],percentage[nivel]))
            break
...
def IMSS(base_salary, prima_vacacional = 0.25, dias_vacaciones = 12, dias_aguinaldo = 15):
    ...
    factor_prima_vacacional = dias_vacaciones*prima_vacacional/365  #60% de los días de vacaciones
...
def bono_anual_desemp(base_salary, p_bono_desemp = 0.08, p_hacienda = 0.25, p_medicare = 0.0145, p_seguro = 0.062):
...
base_salary = 28874.84
monto_vales, excedente = vales_despensa(base_salary, p = 0.1) # vales_despensa(2900, p = 1)
isr = ISR(base_salary + excedente)
cuota_IMSS = IMSS(base_salary, prima_vacacional = 0.6, dias_vacaciones = 12, dias_aguinaldo = 30)
fondo = fondo_ahorro(base_salary)
salario_bruto = base_salary - isr - cuota_IMSS + monto_vales + fondo
bono, impuestos_bono = bono_anual_desemp(base_salary, p_bono_desemp = 0.08, p_hacienda = 0.25, p_medicare = 0.0145, p_seguro = 0.062)
...
salario_total_anual_GE = salario_bruto*12 + bono - impuestos_bono + monto_aguinaldo + monto_vacaciones


base_salary = 30000
salario_bruto = 0
while salario_bruto < 45000:
    base_salary += 100
    monto_vales, excedente = vales_despensa(base_salary, p = 0.0) # vales_despensa(2900, p = 1)
    isr = ISR(base_salary + excedente)
    cuota_IMSS = IMSS(base_salary, prima_vacacional = 0.25, dias_vacaciones = 12, dias_aguinaldo = 15)
    fondo = fondo_ahorro(base_salary, p = 0)
    salario_bruto = base_salary - isr - cuota_IMSS + monto_vales + fondo + 0.1 * base_salary + 0.1 * base_salary
...
for i in range(1, 24):
    ...
    cuota_IMSS = IMSS(ingresos_tributarios, prima_vacacional = 0.6, dias_vacaciones = 12, dias_aguinaldo = 30)
    
    percepcion_mensual = ingresos_tributarios - isr - cuota_IMSS + ingresos_no_tributarios
      
    savings += percepcion_mensual
    
    data.append([date,ingresos_tributarios,ingresos_no_tributarios,percepcion_mensual,isr,cuota_IMSS,monto_vales,fondo,cuenta_fondo,savings])
    
df_income = pd.DataFrame(data, columns = ['Date','Ingresos Tributarios','Ingresos no Tributarios','Percepciones mensual','ISR', 'IMSS', 'Vales Despensa','Fondo mes','Cuenta de Ahorro Fondo','Ahorro'])
...
mask_outliers = np.all((df_B10_GOLD[columns]<1.003) & (df_B10_GOLD[columns]<0.997), axis =1)
# %%
