...
class Wing():
    ...
    def __init__(self,element):
        ...
        self.sections = self.construct_sections(element.find('Sections').findall('Section'))
        self.xml = element
        
    def __repr__(self):
        return "Wing obj:({0}, {1}, No. Sections: {2}])".format(self.name, self.type, self.sections.shape[0])
...
class Airfoil():
    ...
    def __init__(self,airfoil,folder='E:/Documentos/Thesis - Master/Master/XFLR5 exports/airfoils' ):
        self.name = airfoil
        self.path = folder + os.sep + airfoil + '.dat'
        self.data = pd.read_table(self.path,delim_whitespace=True,skiprows=[0],names=['x','y'],index_col=False)
...
def main() -> None:
    ...
    # data
    mytree = ET.parse('XFLR5 exports/N0009_cg0.245_H1.5_V0.10_T2N_VU0.10_Vv0.03_L0.6.xml')
    ...
    transform_coordinates(wing) #Must add method to prevent duplicate transformations -> do not overwrite original 2d airfoil data
